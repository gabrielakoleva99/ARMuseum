...
namespace PaintIn3D
{
	...
	[HelpURL(CwCommon.HelpUrlPrefix + "CwHitCollisions")]
	[AddComponentMenu(PaintCore.CwCommon.ComponentHitMenuPrefix + "Hit Collisions")]
	public class CwHitCollisions : MonoBehaviour
	{
		...
		/// <summary>Hit events are normally sent to all components attached to the current GameObject, but this setting allows you to override that. This is useful if you want to use multiple <b>CwHitCollisions</b> components with different settings and results.</summary>
...
{
	using UnityEditor;
	using TARGET = CwHitCollisions;

	[CanEditMultipleObjects]
	[CustomEditor(typeof(TARGET))]
	public class CwHitCollisions_Editor : CwEditor
	{
		protected override void OnInspector()
		{
			...
			if (Any(tgts, t => t.Emit != CwHitCollisions.EmitType.PointsIn3D))
			{
				BeginIndent();
			...
			BeginIndent();
				if (Any(tgts, t => t.Orientation == CwHitCollisions.OrientationType.CameraUp))
				{
			...
			BeginIndent();
				if (Any(tgts, t => t.PressureMode == CwHitCollisions.PressureType.Constant))
				{
					Draw("pressureConstant", "The pressure value used when PressureMode is set to Constant.", "Constant");
				}
				if (Any(tgts, t => t.PressureMode == CwHitCollisions.PressureType.ImpactSpeed))
				{
			...
			var point    = tgt.Emit == CwHitCollisions.EmitType.PointsIn3D;
			var triangle = tgt.Emit == CwHitCollisions.EmitType.TrianglesIn3D;
			var coord    = tgt.Emit == CwHitCollisions.EmitType.PointsOnUV;
