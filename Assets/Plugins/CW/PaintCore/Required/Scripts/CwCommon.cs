...
namespace PaintCore
{
	...
	public static partial class CwCommon
	{
		...
		public static bool Downsample(RenderTexture renderTexture, int steps, ref RenderTexture temporary)
		{
			if (steps > 0 && renderTexture != null)
			{
				...
				CwCommandReplace.BlitFast(halfRenderTexture, renderTexture, Color.white);
...
namespace PaintCore
{
	...
	public static partial class CwCommon
	{
		...
		private static readonly int _CwShapeTex     = Shader.PropertyToID("_CwShapeTex");
		private static readonly int _CwShapeChannel = Shader.PropertyToID("_CwShapeChannel");

		public static void DrawShapeOutline(Texture shapeTexture, CwChannel shapeChannel, Matrix4x4 shapeMatrix)
		{
			if (shapeTexture != null)
			{
				if (cachedShapeOutline == null)
				{
					cachedShapeOutline = CwHelper.CreateTempMaterial("Modifier Shape", "Hidden/PaintCore/CwShapeOutline");
				}

				cachedShapeOutline.SetTexture(_CwShapeTex, shapeTexture);
				cachedShapeOutline.SetVector(_CwShapeChannel, IndexToVector((int)shapeChannel));
		...
		{
			return FindScriptGUID(typeof(T));
		}

		public static string FindScriptGUID(System.Type t)
		{
			var guids      = AssetDatabase.FindAssets("t:Script " + t.Name);
			...
			foreach (var guid in guids)
			{
				...
				if (System.IO.Path.GetFileName(path) == t.Name + ".cs")
				{
