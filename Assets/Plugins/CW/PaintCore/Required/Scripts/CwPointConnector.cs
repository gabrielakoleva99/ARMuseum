				...
				if (TryGetLink(owner, ref link) == true)
				{
					if (hitSpacing > 0.0f && preview == false)
					{
						...
						for (var i = 0; i < steps; i++)
						{
							...
							if (connectHits == true)
							{
								hitCache.InvokeLine(gameObject, preview, priority, pressure, link.Position, currentPosition, rotation, clipConnected);
							}
							else
							{
					...
					{
						hitCache.InvokeLine(gameObject, preview, priority, pressure, link.Position, position, rotation, clipConnected);
					}
					else
					{
