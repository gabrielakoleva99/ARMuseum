...
namespace PaintCore
{
	/// <summary>This class maintains a list of <b>CwModifier</b> instances, and contains helper methods to apply them.
	...
	public class CwModifierList
	{
		...
		public void DrawEditorLayout(bool showPreviewAndUnique, params string[] groups)
		{
			...
			if (GUILayout.Button("Add Modifier") == true)
			{
				...
				var types = System.AppDomain.CurrentDomain.GetAssemblies().SelectMany(a => a.GetTypes()).Where(t => typeof(CwModifier).IsAssignableFrom(t) && t != typeof(CwModifier));
				...
				foreach (var type in types)
				{
					...
					menu.AddItem(new GUIContent(addGroup + "/" + addTitle + " (" + type.Name + ")"), false, () =>
					{
						...
						modifiers.Add((CwModifier)System.Activator.CreateInstance(addType));
					});
				}
